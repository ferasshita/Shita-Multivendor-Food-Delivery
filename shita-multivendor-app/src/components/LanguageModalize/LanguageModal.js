...
const LanguageModal = ({ modalVisible, setModalVisible, currentTheme, showCrossButton, dontClose }) => {
  ...
  async function determineInitialLanguage() {
    try {
      ...
      const storedLanguageCode = await AsyncStorage.getItem('shita-language')
  ...
  async function onSelectedLanguage() {
    try {
      ...
      await AsyncStorage.setItem('shita-language', languageCode)
      await AsyncStorage.setItem('shita-language-name', languageVal)
