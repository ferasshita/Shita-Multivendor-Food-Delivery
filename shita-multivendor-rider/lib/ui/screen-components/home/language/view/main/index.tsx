...
export default function LanguageMain() {
  ...
  const handleLanguageSelection = async (selectedLanguage: string) => {
    ...
    await AsyncStorage.setItem("shita-language", selectedLanguage);
  };
  const handleSetCurrentLanguage = async () => {
    try {
      const lng = await AsyncStorage.getItem("shita-language");
      if (lng) {
        setIsSelected(lng);
      }
    } catch (error) {
  ...
  const handleSubmission = async () => {
    try {
      ...
      await AsyncStorage.setItem("shita-language", isSelected);
